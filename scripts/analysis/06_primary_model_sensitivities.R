...
ix<-match(d$ForlopsID,a$ForlopsID);stopifnot(!anyNA(ix))
for(v in c("level_L34","level_L45","level_L5S1","stenosis_lateral","stenosis_foraminal"))d[[v]]<-a[[v]][ix]
ids<-c("prior_skeptical","prior_diffuse","gaussian","student_nu5","horseshoe",
       "restricted","operated_level","beta_sv","pure_disc")
make_job<-function(id) {
 full<-d;rhs<-RHS_PRIMARY;p<-pri_zib();f<-zero_inflated_beta();sf<-100
 ...
 if(id=="restricted")rhs<-paste("treatment + age_z + sex + odi_baseline_z + prior_surgery_any",
 ...
 if(id=="horseshoe")p<-c(prior(horseshoe(df=1),class=b),prior(normal(0,3),class=Intercept),
 ...
 if(id %in% c("gaussian","student_nu5")) {
  full$y<-full$odi_3m;dat<-full;sf<-1;p<-pri_odi_gaussian
  form<-bf(as.formula(paste("y | mi() ~",rhs)))
  f<-gaussian()
  if(id=="student_nu5") {f<-student();form<-bf(as.formula(paste("y | mi() ~",rhs)),nu=5)}
 }
 if(id=="beta_sv") {
  n<-sum(!is.na(full$odi_3m));full$y<-((full$odi_3m/100)*(n-1)+.5)/n;dat<-full
  form<-bf(as.formula(paste("y | mi() ~",rhs)));f<-Beta()
  p<-c(prior(normal(0,1),class=b,coef=treatmentELD),prior(normal(0,.5),class=b),
...
run_job<-function(id,cores) {
 b<-make_job(id);dest<-file.path(ROOT,"03_models",paste0("primary_",id,"_calendar"))
 con<-file(file.path(ROOT,"10_logs",paste0("sensitivity_",id,".log")),"wt")
 sink(con);sink(con,type="message");on.exit({sink(type="message");sink();close(con)},add=TRUE)
 ctl<-if(id %in% c("horseshoe","student_nu5"))list(adapt_delta=.99,max_treedepth=14)else list(adapt_delta=.95,max_treedepth=12)
 fit<-brm(b$formula,data=b$data,family=b$family,prior=b$prior,chains=4,iter=2000,warmup=1000,
 ...
 if(!dg$pass){
  ...
  fit<-update(fit,iter=6000,warmup=2000,cores=cores,seed=SEED,
   control=list(adapt_delta=.999,max_treedepth=15),file=NULL,refresh=2000)
  saveRDS(fit,paste0(dest,".rds"));dg<-diagnostics(fit,paste0("sensitivity_",id),15)
 }
 ...
 if(id=="beta_sv") {
  n<-sum(!is.na(d$odi_3m));x$delta<-x$delta*n/(n-1)
...
if(Sys.getenv("ENDO_MODEL_WORKER","")=="1") {
 ...
 manifest<-list();seen<-character()
 for(id in ids){
  b<-make_job(id);code<-make_stancode(b$formula,data=b$data,family=b$family,prior=b$prior)
  fn<-cmdstanr::write_stan_file(code)
  if(!fn %in% seen){cat("Compiling",id,"\n");cmdstanr::cmdstan_model(fn,quiet=TRUE);seen<-c(seen,fn)}
  manifest[[id]]<-data.frame(id=id,stan_file=fn,fit_rows=nrow(b$data),target_rows=nrow(b$target),
    formula=paste(deparse(b$formula$formula),collapse=" "))
 }
 write_csv(do.call(rbind,manifest),"08_qa/sensitivity_models_manifest.csv")
 Sys.setenv(ENDO_MANIFEST="sensitivity_models_manifest.csv",ENDO_SCRIPT="06_primary_model_sensitivities.R",
