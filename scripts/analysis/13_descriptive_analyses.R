...
suppressPackageStartupMessages({library(dplyr);library(tableone)})
...
lc<-raw %>% filter(ForlopsID %in% d$ForlopsID,treatment=='ELD') %>% arrange(surgery_date) %>%
 mutate(case_no=row_number(),operating_time=ifelse(operating_time>=500,NA_real_,operating_time),
 pure_disc=!(stenosis_central==1|stenosis_lateral==1|stenosis_foraminal==1))
stopifnot(nrow(lc)==sum(d$treatment=="ELD"))
pure<-lc %>% filter(pure_disc) %>% mutate(quartile=ntile(case_no,4))
q<-pure %>% group_by(quartile) %>% summarise(n=sum(!is.na(operating_time)),case_no=mean(case_no),
...
rho<-function(x,y,name){v<-suppressWarnings(cor.test(x$case_no,x[[y]],method='spearman',exact=FALSE));
 data.frame(analysis=name,n=sum(!is.na(x[[y]])),rho=unname(v$estimate),p=v$p.value)}
wr(rbind(rho(pure,'operating_time','Pure-disc operating time'),rho(lc,'operating_time','All ELD operating time'),
 rho(pure,'odi_3m','Pure-disc ODI')),'learning_descriptive_verified.csv')
eld<-d[d$treatment=='ELD',];eld$corridor<-ifelse(as.numeric(eld$OpTilgangV3)==1,'Inferred IL','Inferred TF')
stopifnot(all(as.numeric(eld$OpTilgangV3) %in% c(1,3)))
eld$operating_time[eld$operating_time>=500]<-NA_real_
cs<-lapply(c('odi_3m','nrs_leg_3m','operating_time'),function(y){sub<-eld[!is.na(eld[[y]]),];sub$corridor<-factor(sub$corridor,levels=c('Inferred IL','Inferred TF'));
 sub %>% group_by(corridor) %>% summarise(outcome=y,n=n(),mean=mean(.data[[y]]),median=median(.data[[y]]),.groups='drop')})
...
link <- lapply(seq_len(nrow(d)),function(i) {
  ...
  data.frame(ForlopsID=d$ForlopsID[i],first_reop_any_days=earliest(days),
...
re<-bind_rows(lapply(c('Recorded reoperation','Reherniation proxy','Without reherniation flag'),function(y) {
 bind_rows(lapply(c('ELD','MSD'),function(a) {
  s<-ev[ev$treatment==a,];n<-sum(d12$treatment==a)
  ...
  b<-binom.test(k,n);data.frame(outcome=y,treatment=a,k=k,n=n,mean=100*k/n,lower=100*b$conf.int[1],upper=100*b$conf.int[2])
 }))
}))
