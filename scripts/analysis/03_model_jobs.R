# Purpose: Define secondary, operating-time and negative-control models.
# Inputs: Shared configuration and the prepared three- and twelve-month cohorts.
# Outputs: Endpoint job definitions and build_job() for the model runner.

source(file.path(Sys.getenv("ENDO_CODE_DIR"), "00_config.R"))
cache <- file.path(ROOT, "03_models/stan_cache")
...
build_job <- function(j) {
  ...
  if (j$family == "zib") {
    d$y <- pmin(d$y / j$scale, 1 - 1e-6)
    d$baseline_zi <- d[[j$baseline]]
    formula <- bf(as.formula(paste("y ~", RHS)), zi ~ treatment + baseline_zi)
    family <- zero_inflated_beta()
    prior <- pri_zib()
  } else if (j$family == "binary") {
    formula <- bf(as.formula(paste("y ~", RHS)))
    family <- bernoulli()
    prior <- pri_binary
  } else {
    formula <- bf(as.formula(paste("y | mi() ~", RHS)))
    family <- gaussian()
    prior <- switch(j$family,
      eq = pri_eq,
      negative = c(
        prior(normal(0, 2), class = b, coef = treatmentELD), prior(normal(0, 2), class = b),
        prior(student_t(3, 0, 3), class = sigma), prior(normal(2, 2), class = Intercept)
      ),
      optime = c(
        prior(normal(0, 30), class = b, coef = treatmentELD), prior(normal(0, 2), class = b),
