...
for(id in primary_ids) {
 ...
 capture.output(list(formula=fit$formula,prior=prior_summary(fit),
  sampler=fit$fit@stan_args),file=file.path(ROOT,"08_qa",paste0("verified_",id,"_specification.txt")))
 ...
 audit[[id]]<-data.frame(id=id,source=path,fit_rows=nrow(fit$data),
