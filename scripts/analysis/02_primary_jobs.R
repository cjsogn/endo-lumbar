...
build_primary <- function(id) {
 ...
 if(id=="odi_3m") {
  ...
  fam<-zero_inflated_beta();p<-pri_zib();dat<-d[!is.na(d$odi_3m),]
 } else if(id=="los_postop") {
  dat<-d[!is.na(d$los_postop),];dat$los_ordinal<-ordered(pmin(dat$los_postop,3L))
  form<-bf(as.formula(paste("los_ordinal ~",RHS)));fam<-cumulative("logit")
  p<-c(prior(normal(0,2),class=b),prior(normal(0,4),class=Intercept))
 } else {
  dat<-d;form<-bf(as.formula(paste(id,"~",RHS)));fam<-bernoulli();p<-pri_binary
 }
 list(id=id,data=dat,target=d,formula=form,family=fam,prior=p,
