...
run_check<-function(id) {
 ...
 if(grepl("_mi$",id)) {
  ...
  s<-data.frame(mean=pooled$qbar,se=sqrt(pooled$t),lower=pooled$qbar-qt(.975,pooled$df)*sqrt(pooled$t),
  ...
  saveRDS(list(fit=fit,outcome=y,observed=cc,ids=d$ForlopsID[cc],covariates=vars,
   learners=lib,folds=V,seed=SEED),file.path(ROOT,"03_models",paste0("tmle_",id,".rds")))
  s<-extract_result(fit,lower)
  write_csv(data.frame(component=c(rep("Q",length(fit$Qinit$coef)),rep("g",length(fit$g$coef))),
