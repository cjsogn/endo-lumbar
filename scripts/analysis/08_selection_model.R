...
rho_grid<-c(-.10,-.05,0,.05,.10);ids<-paste0("rho_",seq_along(rho_grid))
d<-readRDS(file.path(ROOT,"02_data/derived/cohort_3m.rds"))
# Preserve the submitted selection sensitivity's reduced baseline adjustment
# set. Add the agreed time spline and primary interaction to both regressions.
...
e<-m<-d;e$treatment<-factor("ELD",levels=c("MSD","ELD"));m$treatment<-factor("MSD",levels=c("MSD","ELD"))
dx<-colMeans(model.matrix(f,e)[,-1,drop=FALSE]-model.matrix(f,m)[,-1,drop=FALSE])
obs<-which(!is.na(d$odi_3m));mis<-which(is.na(d$odi_3m))
dat<-list(N=nrow(d),N_obs=length(obs),N_mis=length(mis),K=ncol(X),obs_idx=obs,
...
run_job<-function(id){
 k<-match(id,ids);rho<-rho_grid[k];dat$rho<-rho
 ...
 outdir<-file.path(ROOT,"03_models",paste0("selection_",id));dir.create(outdir,showWarnings=FALSE)
 fit<-mod$sample(data=dat,chains=4,parallel_chains=as.integer(Sys.getenv("ENDO_CHAIN_CORES","4")),
  iter_warmup=1000,iter_sampling=1000,seed=SEED,adapt_delta=.95,max_treedepth=12,refresh=1000,
  output_dir=outdir)
 assess<-function(fit,depth){
  s<-fit$summary();np<-fit$sampler_diagnostics(format="draws_matrix")
  q<-data.frame(id=id,rho=rho,rhat_max=max(s$rhat,na.rm=TRUE),ess_bulk_min=min(s$ess_bulk,na.rm=TRUE),
   ess_tail_min=min(s$ess_tail,na.rm=TRUE),divergences=sum(np[,"divergent__"]),
   max_depth_hits=sum(np[,"treedepth__"]>=depth))
  q$pass<-with(q,rhat_max<1.01&ess_bulk_min>=400&ess_tail_min>=400&divergences==0&max_depth_hits==0);q
 }
 dg<-assess(fit,12)
 if(!dg$pass){
...
if(Sys.getenv("ENDO_MODEL_WORKER","")=="1")for(id in commandArgs(trailingOnly=TRUE))run_job(id)else{
 writeLines(stan,fn);cmdstanr::cmdstan_model(fn,quiet=TRUE)
