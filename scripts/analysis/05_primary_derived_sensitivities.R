...
ev<-function(distance) {rr<-exp(.91*pmax(distance,0)/sd_y);rr+sqrt(rr*(rr-1))}
tab<-do.call(rbind,lapply(list(calendar=x),function(z){
 lo<-unname(quantile(z,.025));hi<-unname(quantile(z,.975))
 data.frame(sd_observed_odi=sd_y,mean=mean(z),lower=lo,upper=hi,
...
N<-nrow(d)
# Algebraically identical to the submitted factual-only mean-shift procedure.
# No bounds clipping was used in the submitted procedure. This is a sensitivity
# calculation on conditional means, not a model for observed ODI outside 0..100.
...
write_csv(data.frame(eld_missing=n_eld,msd_missing=n_msd,target_n=N,
 criterion="PNI <= 0.95 (registered declaration requires > 0.95)",
 first_grid_value=if(length(tipped))min(tipped)else NA_real_,
 grid_max=25,grid_step=.5),"04_results/tipping_point_calendar_summary.csv")
