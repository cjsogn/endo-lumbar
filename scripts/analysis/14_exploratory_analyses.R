# Purpose: Fit subgroup interactions, a causal forest and propensity-weighted contrasts.
# Inputs: Prepared three-month cohort and its saved calendar-basis specification.
# Outputs: Exploratory models, effect summaries, balance and bootstrap diagnostics.
...
read_exploratory_data <- function() {
  ...
  stopifnot(
    nrow(d) == 421L, identical(levels(d$treatment), c("MSD", "ELD")),
...
subgroup_labels <- list(
  age = c("Age <50 years", "Age >=50 years"),
...
build_subgroup <- function(id) {
  ...
  p <- c(
    prior(normal(0, 1), class = b), prior(normal(0, 3), class = Intercept),
...
fit_subgroup <- function(id) {
  b <- build_subgroup(id)
  ncores <- as.integer(Sys.getenv("ENDO_CHAIN_CORES", "4"))
  fn <- file.path(ROOT, "03_models", paste0("exploratory_subgroup_", id, "_calendar"))
  fit <- brm(b$formula,
    data = b$data, family = zero_inflated_beta(), prior = b$prior,
  ...
  if (!dg$pass) {
    ...
    fit <- update(fit,
      iter = 4000, warmup = 2000, cores = ncores, seed = SEED,
  ...
  for (k in seq_along(lev)) {
    ...
    rows[[k]] <- cbind(
      data.frame(subgroup = id, level = lev[k], label = subgroup_labels[[id]][k]),
  ...
  model_int <- cbind(
    data.frame(subgroup = id, coefficient = nm, scale = "mean-model logit"),
  ...
  saveRDS(
    list(
      levels = lev, level_draws = draws, odi_scale_difference = diff,
...
run_subgroups <- function() {
  ...
  for (id in subgroup_ids) {
    b <- build_subgroup(id)
    fn <- cmdstanr::write_stan_file(make_stancode(b$formula,
      data = b$data,
  ...
  Sys.setenv(
    ENDO_MANIFEST = "exploratory_subgroups_manifest.csv",
    ENDO_SCRIPT = "14_exploratory_analyses.R", ENDO_BATCH = "exploratory_subgroups"
  )
  ...
  for (suffix in c("", "_contrast", "_model_interaction")) {
    vals <- lapply(subgroup_ids, function(id) read.csv(file.path(
      ROOT, "04_results",
...
run_forest <- function() {
  d <- read_exploratory_data()
  d <- d[!is.na(d$odi_3m), ]
  # Use continuous baseline predictors in their unstandardised units.
  vars <- sub("_z$", "", COVS)
  X <- model.matrix(reformulate(c(vars, "ct1", "ct2")), data = d)[, -1, drop = FALSE]
  stopifnot(
    all(is.finite(X)), !any(grepl("treatment|odi_3m", colnames(X))),
    all(c("ct1", "ct2") %in% colnames(X))
  )
  W <- as.numeric(d$treatment == "ELD")
  Y <- as.numeric(d$odi_3m)
  set.seed(SEED)
  cf <- causal_forest(
    X = X, Y = Y, W = W, num.trees = 4000, min.node.size = 5,
  ...
  stopifnot(
    length(cate) == nrow(d), all(is.finite(cate)),
    all(is.finite(cf$Y.hat)), all(is.finite(cf$W.hat)),
    all(cf$W.hat > 0 & cf$W.hat < 1)
  )
  summary <- data.frame(
    method = "Causal forest", mean = -unname(ate[1]),
  ...
  mat_table <- function(x) data.frame(
    term = rownames(x), estimate = x[, 1], se = x[, 2],
  ...
  bt <- mat_table(blp)
  bt$estimate <- -bt$estimate
  bt$statistic <- -bt$statistic
  ...
  quant <- data.frame(
    probability = c(0, .025, .25, .5, .75, .975, 1),
  ...
  overlap <- do.call(rbind, lapply(0:1, function(a) data.frame(
    arm = c("MSD", "ELD")[a + 1],
  ...
  saveRDS(list(
    row_index = which(!is.na(read_exploratory_data()$odi_3m)), X = X,
    cate = cate, Y_hat = cf$Y.hat, W_hat = cf$W.hat, tuning = cf$tuning.output,
    calibration = cal, projection = blp
  ), file.path(ROOT, "03_models/exploratory_causal_forest_details.rds"))
  writeLines(
    c(capture.output(cf$tuning.output), capture.output(sessionInfo())),
    file.path(ROOT, "10_logs/exploratory_causal_forest_session.txt")
  )
  cat("Causal forest completed.\n")
}

# Propensities use all cohort procedures, without
# outcome or treatment-derived predictors. Outcome means use observed ODI only.
weight_estimate <- function(X, W, Y) {
  warnings <- character()
  fit <- withCallingHandlers(glm.fit(
    x = X, y = W, family = binomial(),
  ...
  list(
    delta = means[1, ] - means[2, ], means = means, weights = weights, e = e,
...
weight_bootstrap_batch <- function(jobs, design, W, Y, seeds) {
  lapply(jobs, function(k) {
    set.seed(seeds[k])
    ix <- sample.int(length(W), replace = TRUE)
    tryCatch(
      {
        z <- weight_estimate(design[ix, , drop = FALSE], W[ix], Y[ix])
        data.frame(
          replicate = k, capped_iptw = z$delta[1], overlap = z$delta[2],
          warnings = paste(z$warnings, collapse = " | "), error = "", row.names = NULL
        )
      },
      error = function(e) data.frame(
        replicate = k, capped_iptw = NA_real_, overlap = NA_real_,
...
run_weighting <- function() {
  ...
  cl <- parallel::makePSOCKcluster(14L)
  ...
  raw <- parallel::parLapply(
    cl = cl, X = batches, fun = weight_bootstrap_batch,
    design = X, W = W, Y = Y, seeds = seeds
  )
  boot <- do.call(rbind, unlist(raw, recursive = FALSE))
  boot <- boot[order(boot$replicate), ]
  ...
  res <- do.call(rbind, lapply(names(z$weights), function(nm) data.frame(
    method = nm,
  ...
  for (pop in c("full_cohort", "observed_ODI")) {
    ...
    for (nm in c("unweighted", names(z$weights))) {
      ...
      for (a in 0:1) {
        ii <- take & W == a
        diags[[length(diags) + 1]] <- data.frame(
          population = pop, method = nm, arm = c("MSD", "ELD")[a + 1],
  ...
  warnings <- data.frame(
    source = c("cohort propensity", "bootstrap propensity"),
  ...
  write_csv(
    data.frame(
      cap = z$cap, percentile = .99, uncapped_max = max(z$uncapped),
  ...
  writeLines(c(
    "Weighting: pooled 99th-percentile capped stabilised weights and overlap weights.",
    "Intervals: percentile bootstrap, propensity and cap refit per procedure resample.",
    capture.output(sessionInfo())
  ), file.path(ROOT, "10_logs/exploratory_weighting_session.txt"))
...
if (Sys.getenv("ENDO_MODEL_WORKER", "") == "1") {
  ...
  for (mode in modes) switch(mode,
    subgroups = run_subgroups(),
    forest = run_forest(),
    weighting = run_weighting()
  )
  writeLines(capture.output(sessionInfo()), file.path(ROOT, "10_logs/exploratory_session.txt"))
}
