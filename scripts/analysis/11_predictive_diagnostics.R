...
for(id in tab$id) {
 ...
 pred<-pred[,is_observed,drop=FALSE]*sf;y<-y[is_observed]*sf
 ob<-stats(y);sim<-t(apply(pred,1,stats))
 pp[[id]]<-data.frame(id=id,statistic=names(ob),observed=ob,
