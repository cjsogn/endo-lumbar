...
prior_predictive <- function(b) {
 ...
 set.seed(SEED)
 n_draw<-4000L
 sd_mu<-switch(b$job$family,zib=.5,binary=.5,eq=2,negative=2,optime=2)
 tx_sd<-switch(b$job$family,zib=1,binary=1,eq=.5,negative=2,optime=30)
 ic<-switch(b$job$family,zib=c(0,3),binary=c(0,5),eq=c(.5,.5),negative=c(2,2),optime=c(60,30))
 ...
 if(b$job$family=="zib") {
  ...
  mu<-plogis(eta); phi<-rgamma(n_draw,2,.1)
  ...
  sigma_scale<-switch(b$job$family,eq=.3,negative=3,optime=30)
  sigma<-abs(rt(n_draw,3))*sigma_scale
  pred<-eta+matrix(rnorm(length(eta)),n_draw)*sigma
 }
 stats<-data.frame(mean=rowMeans(pred),sd=apply(pred,1,sd),
                  zero_fraction=rowMeans(pred==0))
 saveRDS(stats,file.path(ROOT,"08_qa",paste0(b$job$id,"_prior_predictive.rds")))
 write_csv(data.frame(statistic=names(stats),t(apply(stats,2,quantile,c(.025,.5,.975)))),
...
for(id in names(jobs)) {
 ...
 manifest[[id]]<-data.frame(id=id,family=b$job$family,observed=b$observed,
...
run_job <- function(id,cores) {
 ...
 con<-file(log,"wt");sink(con);sink(con,type="message")
 ...
 if(!dg$pass) {
  ...
  fit<-update(fit,iter=4000,warmup=2000,cores=cores,seed=SEED,
 ...
 s<-cbind(id=id,observed=b$observed,target_n=nrow(b$target),
 ...
 p<-p[,!is.na(obs),drop=FALSE];obs<-obs[!is.na(obs)]
