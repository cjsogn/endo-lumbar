# Purpose: Assemble endpoint summaries and verify model diagnostic records.
# Inputs: Prepared cohorts, combined Bayesian estimates and saved model diagnostics.
# Outputs: Numerical endpoint, association, sampler and convergence summaries.
...
write_csv(data.frame(
  estimate = exp(los$mean), lower = exp(los$lower), upper = exp(los$upper),
  summary = "Exponentiated posterior mean of calendar-averaged conditional log OR for shorter stay; observed-outcome target"
), "04_results/los_odds_ratio.csv")
...
desc <- do.call(rbind, lapply(ids, function(id) {
  ...
  do.call(rbind, lapply(c("MSD", "ELD"), function(a) {
    y <- z[[id]][z$treatment == a]
    y <- y[!is.na(y)]
    data.frame(
      id = id, arm = a, eligible = sum(z$treatment == a), observed = length(y),
...
# Likelihood-ratio checks assess conditional associations after calendar
# adjustment. They cannot exclude confounding from unmeasured anatomy.
fals <- lapply(c("sex", "bmi_z", "education"), function(v) {
  ...
  data.frame(
    variable = v, lr_chi_sq = lr$Deviance[2], df = lr$Df[2], p = lr$`Pr(>Chi)`[2],
...
settings <- lapply(list.files(file.path(ROOT, "03_models"), ".rds$", full.names = TRUE), function(fn) {
  ...
  a <- x$fit@stan_args[[1]]
  data.frame(
    id = basename(fn), chains = length(x$fit@stan_args), iterations = a$iter,
...
print(do.call(rbind, fals))
print(range(dg$rhat_max))
print(range(dg$ess_bulk_min))
