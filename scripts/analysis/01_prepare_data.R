...
d$ct1 <- b[,1]; d$ct2 <- b[,2]
cal <- list(origin=as.Date("2022-01-01"), center=attr(ct,"scaled:center"),
...
write_csv(data.frame(type=c("lower boundary","interior","upper boundary"),
 date=as.character(cal$knot_dates)),"04_results/calendar_knots.csv")
...
write_csv(data.frame(cohort=rep(c("full","eligible_12m"),each=length(CONT)),scaling),
          "04_results/baseline_scaling.csv")
outcomes <- c("odi_3m","odi_12m","nrs_back_3m","nrs_back_12m","nrs_leg_3m","nrs_leg_12m",
...
write_csv(as.data.frame(table(code=factor(codes,levels=sort(unique(c(codes,0:4,9)))),
 useNA="always")),"04_results/eld_approach_codes.csv")
events <- do.call(rbind,lapply(c("perop_comp_any","pt_comp_any_3m"),function(y)
 do.call(rbind,lapply(levels(d$treatment),function(a){
 z<-d[[y]][d$treatment==a]; n<-sum(!is.na(z)); k<-sum(z==1,na.rm=TRUE)
...
n <- sum(!is.na(d$pt_comp_any_3m)); k <- sum(d$pt_comp_any_3m==1,na.rm=TRUE)
write_csv(data.frame(events=k,n=n,overall_rate=k/n,rule="Overall observed cohort event rate",
...
write_csv(data.frame(arm=levels(raw$treatment),
...
dict <- data.frame(variable=names(raw),
...
stopifnot(identical(names(dict),c("variable","class","label","missing")),
          nrow(dict)==ncol(raw),!anyDuplicated(dict$variable))
write_csv(dict,"02_data/derived/data_dictionary.csv")
writeLines(c(capture.output(sessionInfo()),"No patient-level data are printed in logs."),
 file.path(ROOT,"10_logs/data_preparation_session.txt"))
print(counts[counts$outcome %in% c("odi_3m","odi_12m"),]); print(events)
