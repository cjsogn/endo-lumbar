...
suppressPackageStartupMessages({library(brms); library(posterior); library(haven)})
...
SEED <- 20260204L
COVS <- c("age_z", "sex", "bmi_z", "smoking", "education", "employed_baseline",
...
pri_binary <- c(prior(normal(0,1),class=b,coef=treatmentELD),
 prior(normal(0,.5),class=b),prior(normal(0,5),class=Intercept))
pri_eq <- c(prior(normal(0,.5),class=b,coef=treatmentELD),
...
pri_odi_gaussian <- c(prior(normal(0,10),class=b,coef=treatmentELD),
...
diagnostics <- function(fit, id, max_depth=12L) {
 ...
 np <- nuts_params(fit)
 r <- data.frame(id=id, rhat_max=max(relevant$rhat), ess_bulk_min=min(relevant$ess_bulk),
