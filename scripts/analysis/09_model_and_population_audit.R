...
for(fn in list.files(file.path(ROOT,"04_results"),"_calendar_summary.csv$",full.names=TRUE)) {
 ...
 z<-read.csv(fn);new<-rbind(new,z[names(new)])
}
...
for(y in c("odi_3m","odi_12m","day_surgery")) {
 ...
 if(y=="odi_12m") {z$y<-pmin(z$odi_12m/100,1-1e-6);z$baseline_zi<-z$odi_baseline_z}
 obs<-!is.na(z[[y]]);dr<-gcomp(fit,z[obs,],y!="day_surgery",if(y=="day_surgery")1 else 100)
 ...
 targeting[[y]]<-data.frame(id=y,initial_prediction_difference=sign*mean(f$Qinit$Q[,2]-f$Qinit$Q[,1]),
...
comp12<-do.call(rbind,lapply(c("MSD","ELD"),function(a)
 do.call(rbind,lapply(c(TRUE,FALSE),function(observed){
  ...
  data.frame(arm=a,odi_observed=observed,n=nrow(x),variable=CONT,
...
write_csv(data.frame(treatment=d$treatment,propensity=ps,year=format(d$surgery_date,"%Y")),
 "04_results/calendar_propensity_values.csv")
support<-do.call(rbind,lapply(split(seq_len(nrow(d)),d$treatment),function(i)
 data.frame(n=length(i),min_ps=min(ps[i]),q05=quantile(ps[i],.05),median=median(ps[i]),
...
write_csv(as.data.frame(table(arm=d$treatment,year=format(d$surgery_date,"%Y"))),
...
inventory<-do.call(rbind,lapply(fields,function(v){
 x<-raw[[v]]; lab<-attr(x,"label",exact=TRUE);lev<-attr(x,"labels")
 data.frame(field=v,label=if(is.null(lab))""else lab,
...
components<-do.call(rbind,lapply(fields,function(v)do.call(rbind,lapply(c("MSD","ELD"),function(a){
 x<-haven::zap_labels(raw[[v]][raw$treatment==a]);data.frame(field=v,arm=a,
...
dg<-do.call(rbind,lapply(files,read.csv));stopifnot(all(dg$pass))
write_csv(dg,"08_qa/all_calendar_model_diagnostics.csv")
print(do.call(rbind,matched));print(do.call(rbind,targeting))
