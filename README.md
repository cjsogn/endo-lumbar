...
- The primary contrast is MSD minus ELD in ODI units. Positive contrasts favour ELD. Non-inferiority is assessed against the registered 7-point margin, with 5-point and 3-point sensitivity margins. Other contrasts use the endpoint-specific direction and scale in the code. Length of stay uses a cumulative-logit model. Its summary is the conditional log odds ratio for shorter stay averaged over the observed-outcome cohort’s calendar dates. Exponentiation gives the geometric mean conditional odds ratio, not a marginal common odds ratio.
...
The common calendar interaction specification was adopted during revision after the earlier main-effects results were known. It was fixed before these refits and was not chosen separately by endpoint. The primary ODI specification was already in place. TMLE specifications remain as described above.
